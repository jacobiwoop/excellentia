    ...
    <div class="container-fluid py-4">
        <div class="card">
            ...
            <div class="card-header d-flex justify-content-between align-items-center">
                <h2 class="h5 mb-0"><i class="fas fa-receipt me-2"></i>Gestion des Frais</h2>
                                    ...
                                    <td>
                                        @if($fee->type === 'formation')
                                            <button class="btn btn-sm btn-info" data-bs-toggle="modal"
                                                data-bs-target="#modal-{{ $fee->id }}">
                                                Voir détails
                                            </button>
                                        @else
                                            {{ number_format(optional($fee->filieres->first())->pivot->montant ?? 0, 0, ',', ' ') }}
                                            FCFA
                                        @endif
                                    </td>
    ...
    @foreach($fees as $fee)
        @if($fee->type === 'formation')
            <div class="modal fade" id="modal-{{ $fee->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-lg"> <!-- Agrandissement du modal -->
                    <div class="modal-content">
                        <div class="modal-header bg-primary text-white"> <!-- Header coloré -->
                            <h5 class="modal-title fs-4 text-white"><i class="fas fa-receipt me-2 text-white"></i>Détails des frais:
                                {{ $fee->nom }}</h5>
                                    ...
                                    @foreach($fee->filieres as $filiere)
                                        <tr>
                                            ...
                                            <td class="align-middle text-end">
                                                <span class="fw-bold fs-5 text-primary"> <!-- Texte en gras et coloré -->
